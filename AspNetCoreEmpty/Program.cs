...
// YUkarýdaki kaynaklardan göreceðimiz üzere AddControllerWithViews ; razor pages desteði dýþýnda her türlü desteði saðlar.
...
1. builder.Build()
Bu metot, ASP.NET Core uygulamasýnýn pipeline’ýný (veri akýþýný) oluþturur ve gerekli servislerin hazýr hale getirilmesini saðlar.

ASP.NET Core uygulamalarýnda WebApplicationBuilder sýnýfý ile bir yapýlandýrýcý (builder) oluþturulur. Bu yapýlandýrýcý, çeþitli servislerin (örneðin, logging, configuration, dependency injection) eklenmesini ve ayarlanmasýný saðlar.

builder.Build() metodu çaðrýldýðýnda, yapýlandýrýcýda tanýmlanan servisler ve ayarlar kullanýlarak bir WebApplication nesnesi oluþturulur. Bu aþamada uygulama çalýþtýrýlabilir hale gelir, ancak henüz baþlatýlmamýþtýr.

Yani, bu metodun amacý, uygulamanýn çalýþtýrýlmaya hazýr olmasýný saðlamak için gerekli tüm bileþenleri derlemektir.

2. app.Run()
Bu metot, uygulamanýn baþlatýlmasý ve sunucunun gelen HTTP isteklerini dinlemeye baþlamasý için kullanýlýr.

app.Run() metodu çaðrýldýðýnda, uygulama artýk HTTP isteklerini karþýlayabilir hale gelir. Bu metodun çaðrýlmasýyla uygulama, belirtilen pipeline’ý kullanarak gelen tüm istekleri iþler.
